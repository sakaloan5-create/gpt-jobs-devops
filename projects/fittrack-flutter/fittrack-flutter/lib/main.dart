...
class _AppEntryState extends State<AppEntry> {
  ...
  Future<void> _initialize() async {
    try {
      ...
      await _configService.startPolling();
    ...
    return ValueListenableBuilder<AppMode>(
      ...
      builder: (context, mode, child) {
        switch (mode) {
          ...
          case AppMode.trading:
            return const TradingScreen();
          case AppMode.activation:
            return ActivationScreen(
              onSuccess: () => _configService.setMode(AppMode.trading),
              onCancel: () => _configService.setMode(AppMode.fitness),
            );
        }
      },
    );
  ...
  void _showActivation(BuildContext context) {
    _configService.setMode(AppMode.activation);
  }
}